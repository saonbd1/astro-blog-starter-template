---
title: "Muse Glimmer AI: What It Is and How to Run It Locally"
...
Meta released **Muse Glimmer** on August 10, 2026. It is a 30-billion-parameter open-weight model for local AI agents. It focuses on tool use, long tasks, coding, image input, and failure recovery. It runs on a Mac or PC with suitable memory and a compatible runtime. [1] [2]
...
Muse Glimmer is a dense causal language model with a dedicated perception encoder. The perception encoder lets the model read text and images in the same task. This supports screenshot analysis, chart reading, document review, and visual agent workflows. [1] [3]
...
Meta released the model weights under the **Apache 2.0 license**. The weights are available through Hugging Face. The model card describes commercial and research use, subject to the license and usage policy. Read the license and usage policy before you deploy the model in a product. [2] [3]

| Feature | Muse Glimmer detail |
| --- | --- |
| Model size | About 29.6 billion parameters |
| Model type | Dense causal transformer with a perception encoder |
| Input | Text and images |
| Output | Text |
| Context | 131,072 tokens or more, according to the model card |
| License | Apache 2.0 |
| Main focus | Local agents, coding, tool use, and multimodal tasks |
| Reasoning control | Low, medium, high, and xhigh |
| Deployment | Local hardware, servers, and supported inference runtimes |
...
Muse Glimmer targets several tasks that need more than a single answer. It can write and debug code, call tools, inspect images, and continue a long task. Meta also lists synthetic data generation and model evaluation as intended uses. [1] [3]
...
Public setup guidance from Unsloth lists about 17 GB for a 4-bit build, 20–22 GB for a 6-bit build, and 34 GB for an 8-bit build. It lists about 58 GB for BF16. These numbers are guidance for specific builds. Your real need changes with context length, runtime, operating system, and other processes. [4]

| Hardware class | Practical starting point | Best use |
| --- | --- | --- |
| 12–15 GB total memory | Small 2-bit or 3-bit build | Basic tests with lower quality and speed |
| About 17–20 GB | 4-bit build | Local text and agent experiments |
| 24 GB VRAM | K-Quant-17GB style build with working memory | A strong single-GPU starting point |
| 32 GB unified memory or VRAM | Dynamic 4-bit or similar build | Higher quality local work |
| 58 GB or more | BF16 or larger configurations | Research, fine-tuning, and high-memory systems |
...
The GGUF release includes quantized model files. The vision workflow also needs a perception encoder file. The DFlash file is optional and can improve decoding speed. [3]

First, use a llama.cpp version that supports Muse Glimmer. The public GGUF model page states that build `b10353` or newer is required. [3]
...
Ollama is a convenient choice for a local command-line workflow. The GGUF model page lists an Ollama path for the model. vLLM is a better fit for a service that needs an OpenAI-compatible API and concurrent requests. [3]
...
Do not compare quantization names alone. Compare the total memory need, context length, vision files, and runtime overhead. A model file that fits on disk can still fail during generation because the KV cache also needs memory.

## How good is Muse Glimmer for coding and agents?

Meta reports strong results for Muse Glimmer in agentic and coding benchmarks. Its published results include **51.2 on SWE-Bench Pro**, **76.0 on SWE-Bench Verified**, and **51.7 on TerminalBench 2.1**. These are vendor-reported results. [1] [2]
...
| Use case | Why Muse Glimmer is a good candidate | What to test |
| --- | --- | --- |
...
[1]: https://research.meta.ai/blog/introducing-muse-glimmer-open-agentic-model "Meta AI Research: Introducing Muse Glimmer"
[2]: https://developer.meta.com/ai/models/muse-glimmer/ "Meta Developer: Muse Glimmer"
[3]: https://huggingface.co/meta-models/Muse-Glimmer-30B "Hugging Face: Muse Glimmer 30B model card"
[4]: https://unsloth.ai/docs/models/muse-glimmer "Unsloth: Muse Glimmer local setup guide"
[5]: https://developer.nvidia.com/blog/run-local-agentic-ai-workflows-with-metas-muse-glimmer-on-nvidia/ "NVIDIA: Run Local Agentic AI Workflows with Muse Glimmer"
